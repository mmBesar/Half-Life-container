# Dockerfile - Multi-stage for Xash3D Server
...
RUN ./waf configure --build-type=release --dedicated && ./waf build

# Stage 2: HLSDK Master
...
RUN mkdir build
WORKDIR /build/hlsdk-master/build
...
RUN mkdir build
WORKDIR /build/hlsdk-bot10/build
RUN cmake -DCMAKE_BUILD_TYPE=Release ..
RUN cmake --build . --parallel $(nproc)
